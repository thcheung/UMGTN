...
from utils import print_metrics2, labels_to_weights
...
def eval():
    ...
    for _, batch in enumerate(tqdm(test_loader)):
        ...
        with torch.no_grad():
            batch2 = [data.clone() for data in batch]
            batch3 = [data.clone() for data in batch]

            outputs = model(batch)
            _, multi_pred = torch.max(outputs[-1], 1)

            for idx, data in enumerate(batch2):
                batch2[idx].image = torch.zeros_like(data.image).to(data.image.device)
                batch2[idx].image_mask = torch.ones_like(data.image_mask).to(data.image_mask.device)

            outputs = model(batch2)
            _, graph_pred = torch.max(outputs[-1], 1)

            for idx, data in enumerate(batch3):
                ...
                batch3[idx] = data.subgraph(subset)

            outputs = model(batch3)
            _, source_pred = torch.max(outputs[-1], 1)
