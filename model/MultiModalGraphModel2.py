...
import torch
from transformers import AutoModel, ViTModel, BertModel, CLIPVisionModel
...
class MultiModalGraphModel2(nn.Module):
    def __init__(self, hidden_dim=768, label_dim=3, dropout_rate=0.1, language='en'):
        super(MultiModalGraphModel2, self).__init__()
    ...
    def forward(self, data):
        ...
        f0 = self.transformer_multi(x2,src_key_padding_mask=x_mask2)
        ...
        x2 = torch.cat([x0, x1],dim=1)
        x_mask2 = torch.cat([x_mask0,x_mask1],dim=1)
        f1 = self.transformer_multi(x2,src_key_padding_mask=x_mask2)
        ...
        x0 = torch.tanh(self.pooler_multi(f0[:,0]))
        x1 = torch.tanh(self.pooler_multi(f1[:,0]))
        x2 = torch.tanh(self.pooler_multi(f2[:,0]))
